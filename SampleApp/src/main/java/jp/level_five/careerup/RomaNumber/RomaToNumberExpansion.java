...
public class RomaToNumberExpansion extends RomaToNumber {
	...
	public int convert(String str, String string) {
		...
		int number = 0;
		super.pushCharArray(string, charArray);
		for (int i = 0; i <= charArray.length - 1; i++) {
			number += valueIexpand(charArray, i) + valueV(charArray, i)
					+ valueXexpand(charArray, i) + valueL(charArray, i)
					+ valueCexpand(charArray, i) + valueD(charArray, i)
					+ valueM(charArray, i);
		}
	...
	private int valueIexpand(char[] charArray, int i) {
		if ('I' == charArray[i]) {
			if (charArray.length > i
				...
				return -1;
			} else if (charArray.length + 2 > i
					& ('V' == charArray[i + 3] | 'X' == charArray[i + 3]
							| 'L' == charArray[i + 3] | 'C' == charArray[i + 3]
							| 'D' == charArray[i + 3] | 'M' == charArray[i + 3])) {
				return -1;
			} else if (charArray.length + 3 > i
					& ('V' == charArray[i + 4] | 'X' == charArray[i + 4]
							| 'L' == charArray[i + 4] | 'C' == charArray[i + 4]
							| 'D' == charArray[i + 4] | 'M' == charArray[i + 4])) {
				return -1;
			} else if (charArray.length + 4 > i
					& ('V' == charArray[i + 5] | 'X' == charArray[i + 5]
	...
	private int valueCexpand(char[] charArray, int i) {
		if ('C' == charArray[i]) {
			if (charArray.length > i
					& ('D' == charArray[i + 1] | 'M' == charArray[i + 1])) {
				return -100;
	...
	private void checkStringExpand(int i, String string) {
		if ((i < 1 || 3999 < i) || Pattern.matches(".*[VLD]M.*", string)
